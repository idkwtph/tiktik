...
const Discover = () => {
  ...
  return (
    <div className="xl:border-b-2 xl:border-gray-200 pb-6">
      <p className="text-gray-500 font-semibold m-3 mt-4 hidden xl:xl:block">
        Popular Topics
      </p>
      <div className="flex gap-3 flex-wrap">
        {topics.map((item) => (
          <Link href={`/?topic=${item.name}`} key={item.name}>
            ...
            >
              <span className="font-bold text-2xl xl:text-md cursor-pointer">
                {item.icon}
              </span>
              <span className="font-medium text-md hidden xl:block capitalize cursor-pointer">
                {item.name}
              </span>
            </div>
          </Link>
